#In javascript we can assign function to variables. But in Ruby it is not possible directly
#Instead we can create 'Proc' objects to achieve similar behaviors. We can think a Proc is a
...
#In javascript we can pass functions to another functions as arguments but in ruby it is not
#directly achievable. But we can try :)
...
#Every time you want to pass we have to create a proc via "Proc.new". But we can get rid of 
#this convention via '&' operator. Which basically converts a block to a proc or vice-versa

#Here we have &proc. That means we will pass a 'BLOCK' to funcion when called, and our func
#will convert this block to a 'PROC' via '&' operator
